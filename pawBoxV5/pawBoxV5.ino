...
byte kratosAddress = 2; //direccion en memoria donde se guarda el estado del modo god of war
...
dataForController_t getBaseControllerData() {
  ...
  //  values that were in those memory locations before
    ...
    // Botones
    if (kratosMode==1){ //God of War Mode Activation
      controllerData.triangleOn = !digitalRead(l1);
      controllerData.circleOn   = !digitalRead(l2);
      controllerData.squareOn   = !digitalRead(r1);
      controllerData.crossOn    = !digitalRead(r2);
      controllerData.l1On       = !digitalRead(TrianglePin);
      controllerData.l2On       = !digitalRead(CirclePin);
      controllerData.r1On       = !digitalRead(SquarePin);
      controllerData.r2On       = !digitalRead(CrossPin);
  ...
  if (!comboActive) {
      ...
      if (millis() - comboStartTime >= 3000) {
        ...
        //rotation modes
        if (!digitalRead(l2)) {
          EEPROM.update(socdAddress, 5);
          socdMode = 5;  //  actualizamos variable
          
        }
        if (!digitalRead(r2)) {
          EEPROM.update(socdAddress, 6);
          socdMode = 6;  //  actualizamos variable
          
        }
        //God Of War Mode Activation
        if (!digitalRead(UpPin)) {
          EEPROM.update(kratosAddress, 1);
          kratosMode = 1;  //  actualizamos variable
          
        }

        //God Of War Mode Deactivation
        if (!digitalRead(DownPin)) {
          EEPROM.update(kratosAddress, 0);
          kratosMode = 0;  //  actualizamos variable
...
dataForController_t getControllerDataVertical(void){
...
dataForController_t getControllerDataVertical2(void){
...
dataForController_t getControllerDataVerticalDPAD(void){
...
dataForController_t getControllerDataVerticalDPAD2(void){
...
void loop(){
  ...
  if (stickMode != 2) { // analog mode
      switch(socdMode) {
          ...
          case 5: setControllerData(getControllerDataVertical()); break; //rotacion a la izquierda - left rotation
          case 6: setControllerData(getControllerDataVertical2()); break;//rotacion a la derecha - right rotation
      }
  } else { // digital mode
      switch(socdMode) {
          ...
          case 5: setControllerData(getControllerDataVerticalDPAD()); break;//rotacion a la izquierda - left rotation
          case 6: setControllerData(getControllerDataVerticalDPAD2()); break;//rotacion a la derecha - right rotation
      }
  }
