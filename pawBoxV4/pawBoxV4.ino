...
byte leftHand = 0; // si esta en cero se activa el modo para derechos, si esta en uno se actica el modo para zurdos
...
byte handAddress = 3; // direccion en memoria donde se guarda el modo zurdo o derecho
...
dataForController_t getBaseControllerData() {
  ...
  //  values that were in those memory locations before
    ...
    // just the right stick in this firmware
  ...
  if (!comboActive) {
      ...
      if (millis() - comboStartTime >= 3000) {
        ...
        //activamos el modo para zurdos
        if (!digitalRead(RightPin)) {
          EEPROM.update(handAddress, 1);
          leftHand = 1;  //  actualizamos variable
          
        }
        //activamos el modo para derechos, es el que viene por defecto
        if (!digitalRead(LeftPin)) {
          EEPROM.update(handAddress, 0);
          leftHand = 0;  //  actualizamos variable
